...
A network takes dense vectors. Language is a sequence of **discrete
symbols**, and the bridge between them is the simplest layer in the whole
repo: a table with one learned vector per symbol.

It is mathematically identical to multiplying a one-hot matrix by a
weight matrix, and nobody does it that way. For a 50,000-word vocabulary
the one-hot form performs 39 billion multiply-adds of which 99.998% are
by zero. The lookup reads a thousand rows. Both are implemented here so
the comparison can be run rather than asserted.

The forward pass is a memory read with no arithmetic at all. The backward
pass is where the day's content is: gradients must be **scattered** back
to the rows that were read, and a row read twice must receive both
contributions. NumPy's fancy-index assignment silently keeps only the
last write, which under-counts precisely the most common tokens.
"""
...
if __name__ == "__main__":
    ...
    print("an embedding is a lookup, not a matrix multiply:")
    table = Embedding(10, 4, seed=0)
    indices = [3, 7, 3]
    print(f"  table {table.weight.shape}, indices {indices} -> {table(indices).shape}")
    print(f"  row 3 read twice, and both reads are the same vector: "
          f"{np.allclose(table(indices).data[0], table(indices).data[2])}")

    print()
    print("the one-hot form gives identical values...")
    lookup = take_rows(table.weight, indices)
    onehot = one_hot_matmul(table.weight, indices, 10)
    print(f"  identical: {np.allclose(lookup.data, onehot.data)}")

    print("...and identical gradients:")
    a = Embedding(10, 4, seed=0)
    b = Embedding(10, 4, seed=0)
    (take_rows(a.weight, indices) ** 2).sum().backward()
    (one_hot_matmul(b.weight, indices, 10) ** 2).sum().backward()
    print(f"  max difference: {np.abs(a.weight.grad - b.weight.grad).max():.2e}")

    print()
    print("what it costs, at realistic sizes:")
    vocab, dim, tokens = 50000, 768, 1024
    ids = rng.integers(0, vocab, size=tokens)
    big = Tensor(rng.normal(0.0, 0.02, size=(vocab, dim)))
    start = time.perf_counter()
    take_rows(big, ids)
    lookup_ms = (time.perf_counter() - start) * 1000
    start = time.perf_counter()
    one_hot_matmul(big, ids, vocab)
    onehot_ms = (time.perf_counter() - start) * 1000
    useful = tokens * dim
    total = tokens * vocab * dim
    print(f"  vocab {vocab:,}, dim {dim}, {tokens:,} tokens")
    print(f"  lookup   {lookup_ms:>8.2f} ms")
    print(f"  one-hot  {onehot_ms:>8.2f} ms   ({onehot_ms / max(lookup_ms, 1e-9):.0f}x slower)")
    print(f"  the one-hot form does {total:,} multiply-adds, of which")
    print(f"  {useful:,} are not by zero - {100 * useful / total:.4f}% of the work")

    print()
    print("the backward pass is a scatter, and repeats must accumulate:")
    small = Tensor(np.zeros((4, 2)))
    repeated = [1, 1, 1]
    take_rows(small, repeated).sum().backward()
    wrong = np.zeros((4, 2))
    wrong[np.asarray(repeated)] += np.ones((3, 2))
    print(f"  row 1 read 3 times -> np.add.at gives {small.grad[1].tolist()}")
    print(f"  plain fancy-index += gives     {wrong[1].tolist()}")
    print("  NumPy keeps only the last write when an index repeats. In a language")
    print("  model the common words repeat constantly, so the wrong form")
    print("  under-counts exactly the rows that matter most - and still trains.")

    print()
    print("checked against finite differences:")
    for name, function, inputs in (
        ("take_rows", lambda t: (take_rows(t, [2, 0, 2]) ** 2).sum(),
         [rng.normal(size=(4, 3))]),
        ("repeated index", lambda t: (take_rows(t, [1, 1, 1]) ** 2).sum(),
         [rng.normal(size=(3, 2))]),
        ("embedding then linear", lambda t, w: (take_rows(t, [0, 2]) @ w).sum(),
    ...
    print()
    print("an unused row gets exactly zero gradient, so it never moves:")
    table = Tensor(rng.normal(size=(5, 3)))
    (take_rows(table, [0, 1]) ** 2).sum().backward()
    print(f"  rows touched: 0, 1   gradient norms: "
          f"{[round(float(np.linalg.norm(g)), 4) for g in table.grad]}")
    print("  which is why a rare token's embedding is still near its initial value")
    print("  after a whole epoch - Day 3 of NLP-Lab measured how many tokens that is")
